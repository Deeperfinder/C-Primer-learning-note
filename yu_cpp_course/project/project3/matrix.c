...
Matrix * createMat(size_t rows, size_t cols)
{
    ...
    if(p->data == NULL)
    {
        fprintf(stderr, "Faied to allocate memory for the matrix data. \n");
        free(p);  // Don't forget free memory
        return NULL;
    }
    return p;
}
...
{
    // check all parameters carefully first
    ...
    {
        fprintf(stderr, "%s(): Ths 1st parameter has no valid data.\n", __FUNCTION__);
        return false;
    }
    ...
    {
        fprintf(stderr, "%s(): Ths 2nd parameter has no valid data.\n", __FUNCTION__);
        return false;
    }
    ...
    {
        fprintf(stderr, "%s(): Ths 3rd parameter has no valid data.\n", __FUNCTION__);
        return false;
    }
        ...
        input1->cols != input2->cols || input2->cols != output->cols)
    {
        ...
        fprintf(stderr, "Their size are (%zu, %zu), (%zu, %zu) and (%zu, %zu)\n",
